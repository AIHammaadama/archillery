@extends('layouts.admin')

@section('title', 'Procurement Queue | PPMS')

@section('content')
<div class="content-body">
    <div class="container-fluid">
        <!-- Page header -->
        <div class="row page-titles mx-0">
            <div class="col-sm-6 p-md-0">
                <div class="welcome-text">
                    <h4>Procurement Queue</h4>
                    <p class="mb-0">Requests awaiting vendor assignment</p>
                </div>
            </div>
            <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item active">Procurement Queue</li>
                </ol>
            </div>
        </div>

        <!-- Alerts -->
        @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show">
            <i class="bi bi-check-circle me-2"></i>
            {{ session('success') }}
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
        @endif

        <!-- Requests list -->
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Pending Requests ({{ $requests->total() }})</h4>
                    </div>
                    <div class="card-body">
                        @if($requests->count() > 0)
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        <th>Request #</th>
                                        <th>Project</th>
                                        <th>Requested By</th>
                                        <th>Date</th>
                                        <th>Items</th>
                                        <th>Status</th>
                                        <th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($requests as $req)
                                    <tr>
                                        <td>
                                            <a href="{{ route('requests.show', $req) }}" class="fw-bold text-primary">
                                                {{ $req->request_number }}
                                            </a>
                                        </td>
                                        <td>
                                            <div class="fw-bold">{{ $req->project->name }}</div>
                                            <small class="text-muted">{{ $req->project->code }}</small>
                                        </td>
                                        <td>
                                            @if($req->requestedBy)
                                            {{ $req->requestedBy->firstname }} {{ $req->requestedBy->lastname }}
                                            @else
                                            <span class="text-muted">N/A</span>
                                            @endif
                                        </td>
                                        <td>{{ $req->request_date->format('M d, Y') }}</td>
                                        <td>
                                            <span class="badge badge-info">{{ $req->items->count() }} items</span>
                                        </td>
                                        <td>
                                            <span class="badge badge-{{ $req->status->badgeClass() }}">
                                                {{ $req->status->label() }}
                                            </span>
                                        </td>
                                        <td>
                                            <a href="{{ route('approvals.assign-vendors', $req) }}"
                                                class="btn btn-sm btn-primary" title="Assign Vendors">
                                                <i class="bi bi-person-plus"></i> Assign
                                            </a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>

                        <!-- Pagination -->
                        @if($requests->hasPages())
                        <div class="mt-4">
                            {{ $requests->links() }}
                        </div>
                        @endif
                        @else
                        <div class="text-center py-5">
                            <i class="bi bi-inbox display-1 text-muted"></i>
                            <h5 class="mt-3">No Pending Requests</h5>
                            <p class="text-muted">All procurement requests have been processed.</p>
                        </div>
                        @endif
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection